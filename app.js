...
const tracks = {
  se: { label: "Software Engineering", short: "SE", color: "blue", target: 60, unit: "hours", icon: "▰" },
  ai: { label: "Artificial Intelligence", short: "AI", color: "purple", target: 60, unit: "hours", icon: "◇" },
  ielts: { label: "IELTS", short: "IELTS", color: "yellow", target: 112, unit: "hours", icon: "◉" },
  app: { label: "App Launch", short: "APP", color: "orange", target: 100, unit: "users", icon: "▱" },
  channel: { label: "Personal Channel", short: "CHANNEL", color: "cyan", target: 1000, unit: "subscribers", icon: "◌" },
  jobs: { label: "Job Pipeline", short: "JOB", color: "green", target: 2, unit: "interviews", icon: "↗" }
};
const baseTasks = {
...
const reviewTask = { track: "review", label: "REVIEW", title: "Đánh giá ngày, ghi nhận năng lượng và chọn ưu tiên ngày mai", output: "Daily review with energy, focus, biggest win, blocker, and tomorrow's priority", minutes: 30, priority: "P1" };
...
function safeLoad() {
  try {
    ...
    ["completed", "minutes", "evidence", "outcomesByDay", "reviews"].forEach(k => {
...
function dayData(d) {
  const day = Number(d); const idx = (day - 1) % 7; const week = Math.ceil(day / 7);
  return { d: day, week, idx, date: dateForDay(day), weekday: dateLabel(day), tasks: [...Object.keys(tracks).map(t => {
    const phase = week === 1 ? "baseline" : week === 2 ? "build" : week === 3 ? "validate" : "ship";
    return { track: t, title: `${baseTasks[t][idx]} · ${phase} W${week} / Day ${String(day).padStart(2, "0")}`, output: `${outputs[t][idx]} · day-${String(day).padStart(2, "0")}`, minutes: t === "ielts" ? 240 : t === "se" || t === "ai" ? 130 : t === "app" ? 90 : t === "channel" ? 75 : 60, priority: idx === 0 || week === 4 ? "P0" : idx < 3 ? "P1" : "P2" };
  }), { ...reviewTask, title: `${reviewTask.title} · Day ${String(day).padStart(2, "0")}`, output: `${reviewTask.output} · day-${String(day).padStart(2, "0")}` }] };
}
...
function taskHtml(t, d = saved.selectedDay) {
  ...
  return `<div class="task-line ${done ? "done" : ""}"><button class="check ${done ? "done" : ""}" data-task="${t.track}">${done ? "✓" : ""}</button><div class="task-text">${t.title}<div class="task-track"><span class="track-dot ${t.track}"></span>${meta.label} · ${t.output}</div><input class="input evidence-input" data-evidence="${t.track}" value="${evidence}" placeholder="Evidence / note (optional)"></div><span class="task-time">${t.minutes}m</span></div>`;
}
function trackCards() {
  return Object.entries(tracks).map(([t, x]) => {
...
function schedule() {
  ...
  return `<div class="view"><div class="page-heading"><div><div class="eyebrow">DAILY EXECUTION</div><h1>Schedule – Day ${d}</h1><p>Arrange tasks by priority and energy. AI can suggest optimizations.</p></div><div class="day-switch"><button data-prev-schedule>‹</button><span>${dateLabel(d)}</span><button data-next-schedule>›</button></div></div><div class="schedule-header"><h2>${dateLabel(d)}</h2><div class="schedule-controls"><button data-ai-optimize>🤖 AI Optimize</button><button data-reset-schedule>↻ Reset</button></div></div>${suggestions.length > 0 ? `<div class="ai-suggestions"><div class="ai-suggestions-title"><span class="ai-icon">💡</span>AI Suggestions</div>${suggestions.map(s => `<div class="suggestion-item"><div class="suggestion-type">${s.type}</div><div class="suggestion-msg">${s.message}</div><div class="suggestion-rec">${s.recommendation}</div></div>`).join("")}</div>` : ""}<table class="schedule-table"><thead><tr><th>TIME</th><th>TASK</th><th style="width:80px">PRIORITY</th><th style="width:80px">DURATION</th><th style="width:50px">✓</th></tr></thead><tbody>${daySchedule.map((task, i) => `<tr class="task-row" draggable="true" data-task-index="${i}"><td class="time-cell">${task.startTime} — ${task.endTime}</td><td class="task-cell"><div class="task-title">${task.title}</div><div class="task-output">${task.output}</div></td><td><span class="priority-badge ${task.priority.toLowerCase()}">${task.priority}</span></td><td class="duration-cell">${task.minutes}min</td><td class="status-cell"><input type="checkbox" class="check inline" ${saved.completed[key(d, task.track)] ? "checked" : ""} data-task-complete="${key(d, task.track)}"></td></tr>`).join("")}</tbody></table><div class="total-time"><span>Total time on tasks:</span><span class="total-time-value">${Math.floor(totalDuration / 60)}h ${totalDuration % 60}m</span></div><div class="schedule-footer"><p><strong>💡 Tips:</strong></p><ul style="margin:8px 0;padding-left:20px;font-size:11px"><li>Drag tasks to reorder by priority and energy</li><li>P0 tasks should be first (morning energy peak)</li><li>Long tasks should be split with breaks</li><li>Click "AI Optimize" to get schedule suggestions</li><li>Check tasks as you complete them</li></ul></div></div>`;
}
...
function headerTitle(view) { return ({ dashboard: "Dashboard", calendar: "Calendar", today: "Today", schedule: "Schedule", se: "Software Engineering", ai: "Artificial Intelligence", ielts: "IELTS", app: "App Launch", channel: "Personal Channel", jobs: "Job Pipeline", analytics: "Analytics", reviews: "Reviews", settings: "Settings" })[view] || "Dashboard"; }
function render(view = "dashboard") { const active = document.querySelector(".nav-item.active"); if (active && view === "dashboard") view = active.dataset.view; document.querySelector("#page-label").textContent = headerTitle(view); document.querySelector("#view-root").innerHTML = (views[view] || views.dashboard)(); updateChrome(); }
...
function bindEvents() {
  document.addEventListener("click", e => {
    ...
    const task = e.target.closest("[data-task]"); if (task) { saved.completed[key(saved.selectedDay, task.dataset.task)] = !saved.completed[key(saved.selectedDay, task.dataset.task)]; saveAndRender("Task status saved"); return; }
    const taskComplete = e.target.closest("[data-task-complete]"); if (taskComplete) { const k = taskComplete.dataset.taskComplete; saved.completed[k] = taskComplete.checked; persist(); showToast("Task status saved"); return; }
    const resetSched = e.target.closest("[data-reset-schedule]"); if (resetSched) { delete saved.schedule[saved.selectedDay]; saveAndRender("Schedule reset to default"); return; }
  ...
  document.addEventListener("drop", e => {
    ...
    if (row && draggedIndex !== null) {
      ...
      const dayTasks = dayData(d).tasks;
      const daySchedule = saved.schedule[d] || ScheduleEngine.generateDaySchedule(d, dayTasks, saved.settings);
      const newSchedule = ScheduleEngine.moveTask(daySchedule, draggedIndex, toIndex);
